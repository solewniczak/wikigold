...
def import_dump_command(dir, tag):
    ...
    sql_add_line = "INSERT INTO `lines` (`article_id`, `nr`) VALUES (%s, %s)"
    sql_add_token = "INSERT INTO `tokens` (`line_id`, `nr`, `token`) VALUES (%s, %s, %s)"
    for title, lines in dump_parser.parse_xml(pages_meta_current_filepath, all_titles_in_ns0, all_titles_count, 1000):
        ...
        for line_nr, tokens in enumerate(lines):
            data_line = (article_id, line_nr)
            cursor.execute(sql_add_line, data_line)
            line_id = cursor.lastrowid
            for token_nr, token in enumerate(tokens):
                data_token = (line_id, token_nr, token)
                cursor.execute(sql_add_token, data_token)
